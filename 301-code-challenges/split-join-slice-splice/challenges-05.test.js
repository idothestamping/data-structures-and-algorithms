...
const splitFoods = (recipe) => {
  ...
  for(let i = 0; i < recipe.ingredients.length; i++){
    ...
    tempStr.shift();
    tempStr.shift();
    //concatenate and add to the result
    result.push(tempStr.join(' '));
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  //Return empty if more than string length
  if(numberOfCharacters > str.length){
    return '';
  }
  //Return the original string if negative
  if(numberOfCharacters < 0){
    return str;
  }
  //split the character strings
  let arr = str.split('');
...
const removeVowels = (str) => {
  ...
  while(index < chars.length){
    if(vowels.includes(chars[index].toLowerCase())){
...
const extractVowels = (str) => {
  ...
  while(index < chars.length){
    if(vowels.includes(chars[index].toLowerCase())){
      removedVowels.push(chars[index]);
      chars.splice(index, 1);
    } else{
      index++;
    }
  }
  //sort the removed vowels
  removedVowels.sort();
  //return the array result
  return[chars.join(''), removedVowels.join('')];
};
